...
public class NowPlaying extends AppCompatActivity {
    ...
    Button play_now_play;
    Button play_previous;
    Button play_next;
...
//        ImageView album_art = (ImageView) findViewById(R.id.art_main);
        ...
        //Buttons
        play_now_play = (Button) findViewById(R.id.now_playing);
        play_previous = (Button) findViewById(R.id.previous_now);
        play_next = (Button) findViewById(R.id.forward_now);


        if(ms.player.isPlaying())
        {
            Drawable pause = AppCompatResources.getDrawable(getApplicationContext(), R.drawable.pause);
            play_now_play.setBackground(pause);
        }
        else
        {
            Drawable play = AppCompatResources.getDrawable(getApplicationContext(), R.drawable.ic_play_arrow_black_24dp);
            play_now_play.setBackground(play);
        }
        ...
        play_now_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(ms.player.isPlaying())
                    {
                        ms.pause();
                        Drawable pause = AppCompatResources.getDrawable(getApplicationContext(), R.drawable.ic_play_arrow_black_24dp);
                        play_now_play.setBackground(pause);
                    }
                else {
                    ms.play();
                    Drawable play = AppCompatResources.getDrawable(getApplicationContext(), R.drawable.pause);
                    play_now_play.setBackground(play);
                }
            }
        });
